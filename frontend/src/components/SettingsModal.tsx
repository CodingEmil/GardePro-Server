...
export function SettingsModal({ onClose }: Props) {
  ...
  const handleBtWake = async () => {
    ...
    // Animate step labels while the request is in flight
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-surface border border-border rounded-xl p-6 w-96 shadow-2xl">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-text font-semibold">⚙ Einstellungen</h2>
        ...
        <div className="mb-4 min-h-[1.5rem]">
          ...
          {btStatus === 'ok' && (
            <p className="text-green-400 text-xs">
              WLAN der Kamera wurde erfolgreich aktiviert.
            </p>
          )}
          {btStatus === 'error' && (
            <p className="text-red-400 text-xs">{btError}</p>
          )}
        </div>
        ...
        <div className="border-t border-border mt-4 pt-4 mb-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted mt-1">Thumbnails von der Kamera statt lokal generieren.</p>
            </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 border border-border text-muted rounded py-1.5 text-sm hover:border-accent hover:text-accent transition-colors"
          >
            Abbrechen
          </button>
          <button
            onClick={handleSave}
            className="flex-1 bg-accent text-bg rounded py-1.5 text-sm font-medium hover:bg-accent-dim transition-colors"
          >
            Speichern
          </button>
        </div>
